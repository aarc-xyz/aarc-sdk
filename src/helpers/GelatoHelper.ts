...
export const getGelatoTransactionStatus = async (
  gelatoTxStatusDto: GelatoTxStatusDto,
  ...
  while (
    response != undefined &&
    (response.taskState === TaskState.CheckPending ||
      response.taskState === TaskState.ExecPending)
  ) {
  ...
  if (
    response != undefined &&
    response.taskState === TaskState.WaitingForConfirmation
  ) {
    return response.transactionHash;
  } else if (
    response != undefined &&
    (response.taskState === TaskState.ExecReverted ||
      response.taskState === TaskState.Cancelled)
  ) {
    Logger.log('Transaction failed');
    return false;
  } else if (
    response != undefined &&
    response.taskState === TaskState.ExecSuccess
  ) {
    return response.transactionHash;
  } else {
    return false;
