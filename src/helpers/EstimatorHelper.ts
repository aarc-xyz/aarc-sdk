...
import { GAS_UNITS, GAS_TOKEN_ADDRESSES } from '../utils/Constants';
...
export const calculateTotalGasNeeded = async (
  ...
  for (const transaction of transactions) {
    try {
      if (transaction.to !== GAS_TOKEN_ADDRESSES[chainId as ChainId]) {
      ...
      const tx = {
        ...
        to:
          transaction.to !== GAS_TOKEN_ADDRESSES[chainId as ChainId]
            ? transaction.tokenAddress
            : transaction.to,
